...
public:
	...
	static std::vector<uint64_t> ScanAllRaw(uint64_t start_address, std::vector<uint8_t> sig_bytes, std::vector<char> sig_wildcards, bool start_address_is_restrict_allocation_base, DWORD scan_protection = PAGE_EXECUTE_READ | PAGE_EXECUTE_READWRITE);
